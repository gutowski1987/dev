// module with budget calculations-------------------------------------------------------------------------------------------
var budgetController = (function() {
    ...
    return {
        addItem: function(type, des, val) {
            ...
            // Create new item exp/inc
            if (type === 'exp') {
                newItem = new Expense(ID, des, val);
            } else if (type === 'inc') {
                newItem = new Income(ID, des, val);
            }
            // Adding item do the exp/inc array in data structure
            data.allItems[type].push(newItem);
            //return newItem for use
            return newItem;
        },
        deleteItem: function(type, id) {
            var ids, index;
            // select the id from the array where we have all inc/exp
            ids = data.allItems[type].map(function(current) {
                return current.id;
            });
            // select the id what we want to delete by id of the inc/exp
            index = ids.indexOf(id);
            // delete item from data array of inc/exp
            if (index !== -1) {
        ...
        getInput: function() {
            return {
                ...
                description: document.querySelector(DOMstrings.inputDescription)
                    .value, // description of inc/exp
                value: parseFloat(
                    document.querySelector(DOMstrings.inputValue).value
                ) // number value of the inc/exp
            };
        },
...
var appController = (function(budgetCtrl, UICtrl) {
    ...
    var ctrlDeleteItem = function(event) {
        ...
        if (itemID) {
            ...
            // 1. Del item from data structure
